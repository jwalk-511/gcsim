...
func (c *char) Burst(p map[string]int) (action.Info, error) {
	...
	// TODO: When does this really start counting?
	c.displacement = c.Core.Player.Displacement()
	c.updateATKBuff()
...
func (c *char) applyBuffTask(src int) {
	c.Core.Tasks.Add(func() {
		...
		points := c.burstMovementRestoreNS()
...
func (c *char) burstMovementRestoreNS() float64 {
	displacement := c.Core.Player.Displacement()
	delta := displacement - c.displacement
	c.displacement = displacement

	// Get 2 points per m moved
	return delta/2.0 + c.a1Points() + c.c4Points()
}
